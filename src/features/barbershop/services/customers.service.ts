...
export const customersService = {
  async getList(options?: { search?: string; sort?: CustomerSort }) {
    ...
    if (error || !response) throw new Error(error?.value?.message || "Failed to fetch customers");
    return response.data || [];
  },

  async getById(id: string) {
    const { data: response, error } = await app.api.customers({ id }).get();
    if (error || !response) throw new Error(error?.value?.message || "Failed to fetch customer");
    return response.data;
  },

  async getBookings(id: string) {
    const { data: response, error } = await app.api.customers({ id }).bookings.get();
    if (error || !response) throw new Error(error?.value?.message || "Failed to fetch customer bookings");
    return response.data || [];
  },
};
